...
def main():
    ...
    for pair in match_list:
        ...
        if in_different_set(pair, set1, set2) and\
            ...
            # print(f"Add cut edge: {pair.id1:4d} <--> {pair.id2:4d},\tscore: {pair.score}")
            print(f"Cut edge: {img_list[pair.id1]} <----> {img_list[pair.id2]},\tscore: {pair.score}")
