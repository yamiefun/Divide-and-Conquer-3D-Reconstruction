...
def calculate_err(mtx, matched_set, blk1, blk2):
    err = 0
    dist_thresh = 1.5
    outlier = 0
...
def calculate_cam_err(mtx, pnt_set1, pnt_set2):
    ...
    out_ratio = outlier / len(pnt_set1)
    print(f"Inlier: {inlier}, Outlier: {outlier}")
    print(f"Outlier Ratio: {out_ratio*100:.3f}%")

    return outlier


def calculate_cam_err2(mtx, pnt_set1, pnt_set2, thresh):
    assert len(pnt_set1) == len(pnt_set2),\
        f"Different anchor images."
    dist_thresh = thresh
    outlier = 0
    ...
    # print(f"Inlier: {inlier}, Outlier: {outlier}")
    # print(f"Outlier Ratio: {out_ratio*100:.3f}%")
    avg_dist = total_dist/len(pnt_set1)

    return avg_dist
    # return out_ratio


def calculate_cam_err3(mtx, pnt_set1, pnt_set2, thresh):
    assert len(pnt_set1) == len(pnt_set2),\
        f"Different anchor images."
    outlier = 0
    inlier = 0
